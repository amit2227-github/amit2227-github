...
**1. Upload the two custom SVGs** — `boot-sequence.svg` and `skill-graph.svg` — to the root of your `amit2227-github` repo, same folder as `README.md`. These are one-of-a-kind assets built for you; the README references them by relative path.
...
Commit → **Actions** tab → run the workflow once. Refreshes daily on its own after that.
